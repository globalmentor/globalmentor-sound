...
public class SampledSounds {
	...
	/**
	 * Constructs and returns a data line of the specified type from the given input stream. If a <code>Clip</code> is returned it will already be open. The input
	 * stream should be one that allows marking so that <code>AudioSystem.getAudioFileFormat()</code> can correctly extract information before the audio is
	 * actually read.
	 * @param inputStream The input stream of the audio.
	 * @param lineClass The type of line desired (for example <code>Clip.class</code>).
	 * @return A data line from the input stream. This line will already be open if the line is a <code>Clip</code>.
	 * @throws IOException Thrown if there is an error reading from the given input stream.
	 * @throws UnsupportedAudioFileException Thrown if the audio file represented by the input stream is not supported.
	 * @throws LineUnavailableException Thrown if the line is not available because, for example, it is already in use by another application.
	 */
	public static DataLine getDataLine(final InputStream inputStream, final Class<?> lineClass) throws IOException, UnsupportedAudioFileException,
	...
	protected static void play(final InputStream inputStream) throws IOException, UnsupportedAudioFileException, LineUnavailableException { //TODO unfinished; finish or delete
		...
		AudioFormat conversionAudioFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, audioFormat.getSampleRate(), 16, audioFormat.getChannels(),
